...
import { getWithdrawalFee } from "src/utils/vault";

export const useGetVaultWithdrawalFee = (
  ...
  const fetchFee = useCallback(async () => {
    if (account) {
      const { withdrawFee, denom } = await getWithdrawalFee(
        ethereum,
        vaultRewardsAddress
      );
      ...
      const calculatedWithdrawalFee = withdrawFeeBN
        .div(denomBN)
        .multipliedBy(100);
